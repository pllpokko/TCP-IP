...
void write_routine(int sock, char *buf,int now_client_sock,char *ID);
...
//	write(sock, month2, sizeof(month2));
	...
	pid = fork();
	if (pid == 0)
		write_routine(sock, buf,now_client_sock,ID);
	else
...
void write_routine(int sock, char *buf,int now_client_sock,char* ID)
{
	...
	char user[21+BUF_SIZE];
	sprintf(user,"[%s]",ID);
	sprintf(user,"%s %s",user,buf);
