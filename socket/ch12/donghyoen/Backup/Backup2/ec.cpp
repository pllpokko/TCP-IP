...
void write_routine(int sock, char *buf)
{
	...
	if(!strcmp(buf,"move to another month\n") || !strcmp(buf,"Move to another month\n"))
	{
		int p,q;
		char year6[5],month6[3];
		puts("What do you want to see? ex)year month\n");
		fflush(stdin);
		fgets(buf,BUF_SIZE,stdin);
//		strncpy(year6,buf,4);
		for(p=0;p<4;p++)
